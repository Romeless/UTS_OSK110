...
def hysteresis_corner(img):
    ...
    for i in range(1, nrow-2):
        ...
        if img[i, 1] == 0:
            img[i, 1] = 255
            (img, rec, last_i, last_j, limit) = spreading(img, i, 0, rec, False)
        if img[i, ncol-2] == 0:
            img[i, ncol-2] = 255
            (img, rec, last_i, last_j, limit) = spreading(img, i, ncol-2, rec, False)
        while limit:
            print("limit bang?")
            rec = 0
    ...
    for j in range(1, ncol-2):
        ...
        if img[1, j] == 0:
            img[1, j] = 255
            (img, rec, last_i, last_j, limit) = spreading(img, 0, j, rec, False)
        if img[nrow-2, j] == 0:
            img[nrow-2, j] = 255
            (img, rec, last_i, last_j, limit) = spreading(img, nrow-2, j, rec, False)
        while limit:
            print("limit bang?")
            rec = 0
            img[last_i, last_j] = 255
